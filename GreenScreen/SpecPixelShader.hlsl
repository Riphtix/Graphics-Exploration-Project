Texture2D mytextures[2] : register(t0);
...
{
    float4 surface = mytextures[0].Sample(samplerState, input.uvw.xy);
    float4 specMap = mytextures[1].Sample(samplerState, input.uvw.xy);
    
    // Directional light
    float3 dLightPosition = { -1, 0, -0.2 };
    float dLightRatio = saturate(dot(-normalize(dLightPosition), normalize(input.nrm)));
    float4 dColor = { 1.0f, 1.0f, 1.0f, 0.4f };
    ...
    float specResult = specLighting * specMap;
    
    surface.a = 1.0f;
    float4 ambiant = { 0.3, 0.3, 0.3, 1 };
    float4 color = ambiant * saturate(surface + pLightResult + dLightResult + sLightResult + specResult);
    return color;
}